...
<div class="w-full flex justify-end p-2 text-[#d3d6dff]">
    <select name="locale" id="locale">
        <option value="en" @if (\App::isLocale('en')) selected @endif>EN</option>
        <option value="de" @if (\App::isLocale('de')) selected @endif>DE</option>
        <option value="">{{ App::currentLocale() }}</option>
    </select>
</div>
...
<script>
    $('#locale').on('change', function() {
        ...
        window.location.href = "{{ route('setlocale') . '?' }}" + params;
    });
</script>
