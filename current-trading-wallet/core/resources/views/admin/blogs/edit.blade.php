        ...
        <div class="w-full flex justify-center">
            <div class="w-11/12 rounded-md bg-[#0e1726] p-2 md:p-4">
                ...
                <div class="p-2 md:p-4">
                    <form class="mt-2 p-2 md:p-4" action="{{ route('admin.blogs.edit-validate', $blog->id) }}"
                        ...
                        <div class="text-[#bfc9d4] text-xs md:text-sm">
                            <div class="w-full">
                                ...
                                <textarea
                                    class="w-full pt-2 bg-[#0e1726] transition-colors duration-200 transform focus:outline-none border-b-2 border-slate-800 focus:border-b-slate-500"
                                    name="snippet" id="snippet">{{ old('snippet') ?? $blog->snippet }}</textarea>
                            </div>
                            <span class="p-1 text-red-600">
                                @error('snippet')
                                    {{ $message }}
                        ...
                        <div class="text-[#bfc9d4] text-xs md:text-sm">
                            <div class="w-full ck-h-400">
                                ...
                                <textarea
                                    class="w-full pt-2 bg-[#0e1726] transition-colors duration-200 transform focus:outline-none border-b-2 border-slate-800 focus:border-b-slate-500"
                                    name="detail" id="detail">{!! old('detail') ?? $blog->detail !!}</textarea>
                            </div>
                        ...
                        <div class="text-[#bfc9d4] text-xs md:text-sm mt-2">
                            <div class="w-full flex items-baseline space-x-1">
                                <label class="font-medium w-28 overflow-hidden" for="cover_image">Cover Image:</label>
                                <label
                                    ...
                                    <span id="cover-image-preview"
                                        ...
                                        style="background-image: url(@if($blog->type == 'auto') {{ $blog->img }}  @else {{ route('file', ['blogs', $blog->img]) }} @endif); background-size: contain; background-repeat: no-repeat;">
