...
use App\Http\Controllers\User\OwnedCoinStakingController;
use App\Http\Controllers\User\OwnedDemoController;
use App\Http\Controllers\User\OwnedTradeController;
use App\Http\Controllers\User\OwnedTradingWalletController;
...
Route::prefix('user')->name('user.')->middleware(['onlyLoggedIn', 'loginOtpCheck'])->group(function () {
    ...
    // First-party CryptoTrading replacement routes. These preserve the public
    // route names used by the existing Blade templates while avoiding encoded
    // module controllers and entity classes.
    Route::prefix('trading')->name('trading.')->group(function () {
        Route::prefix('wallets')->name('wallets.')->group(function () {
            Route::get('/', [OwnedTradingWalletController::class, 'index'])->name('index');
            Route::post('create', [OwnedTradingWalletController::class, 'create'])->name('create');
            Route::post('fund-withdraw', [OwnedTradingWalletController::class, 'fundWithdraw'])->name('fund-withdraw');
            Route::get('{address}', [OwnedTradingWalletController::class, 'view'])->name('view');
        });

        Route::prefix('trade')->name('trade.')->group(function () {
            Route::get('index', [OwnedTradeController::class, 'index'])->name('index');
            Route::post('index', [OwnedTradeController::class, 'tradeValidate'])->name('trade-validate');
            Route::post('end-trade', [OwnedTradeController::class, 'endTrade'])->name('end-trade');
            Route::post('bot-activate', [OwnedTradeController::class, 'botActivate'])->name('bot-activate');
            Route::post('bot-trade', [OwnedTradeController::class, 'botTrade'])->name('bot-trade-validate');
            Route::get('bot/{symbol1}/{symbol2}', [OwnedTradeController::class, 'bot'])->name('bot');
            Route::get('{symbol1}/{symbol2}', [OwnedTradeController::class, 'trade'])->name('trade');
        });

        Route::prefix('demo')->name('demo.')->group(function () {
            Route::get('/', [OwnedDemoController::class, 'index'])->name('index');
            Route::post('/', [OwnedDemoController::class, 'endTrade'])->name('end-trade');
            Route::post('index', [OwnedDemoController::class, 'tradeValidate'])->name('trade-validate');
            Route::get('{symbol1}/{symbol2}', [OwnedDemoController::class, 'trade'])->name('trade');
        });

        Route::get('coin-stakings', [OwnedCoinStakingController::class, 'index'])->name('coin-stakings.index');
        Route::post('coin-stakings', [OwnedCoinStakingController::class, 'stake'])->name('coin-stakings.stake');
    });
