			...
			<div class="w-full">
				...
				<div class="mt-4 mb-2">
					...
					<div class="w-full flex justify-start items-center space-x-5">
						{{--  confirm --}}
						<div>
							<button type="button" onclick="payWithMonnify()" class="text-xs md:text-sm text-[#d1d5db] text-center px-5 py-2 bg-[#1b2e4b] hover:bg-gray-700 rounded-md">
								{{ ct('Pay now') }}
							</button>
						</div>
...
<form id="monnify-cancel-form" action="{{ route('user.deposit.pay.manual.cancel') }}" method="POST">
	@csrf
</form>
@endsection
...
<script type="text/javascript">
	function payWithMonnify() {
		MonnifySDK.initialize({
			amount: "{{ round(session()->get('converted_amount')) }}",
