    ...
    <div class="w-full flex justify-center">
        <div class="w-11/12 rounded-sm bg-[#0e1726] p-1 md:p-4">
            ...
            <div class="p-2 md:p-4">
                <form id="submitForm" class="mt-2 p-2 md:p-4" action="{{ route('admin.settings.custom-js-validate') }}" method="post" enctype="multipart/form-data">
                    ...
                    <div class="text-[#bfc9d4] text-xs md:text-sm">
                        <div class="w-full flex items-baseline space-x-1">
                            <textarea cols="30" rows="10" name="custom_js" id="custom_js" required>{!! json_decode(websiteInfo('custom_js')) !!}</textarea>


                        </div>
                        <span class="p-1 text-red-600">
                            @error('custom_js') {{ $message }} @enderror
                        </span>
                        <span id="failed"></span>
                    </div>
                    ...
                    <div class="w-full my-5 px-5">
                        <button type="submit" id="submit" class="w-full text-xs md:text-sm text-[#d1d5db] text-center px-5 py-3 my-5 bg-[#1b2e4b] hover:bg-gray-700 rounded-md">
                            Save Changes
                        </button>
                    </div>
                </form>
...
<script type="text/javascript">
    ...
    $('#submitForm').on('submit', function(e) {
        ...
        let custom_js = $('#custom_js').val();
        $.ajax({
            url: "{{ route('admin.settings.custom-js-validate') }}",
            type: "POST",
            data: {
                "_token": "{{ csrf_token() }}",
                custom_js: custom_js,
